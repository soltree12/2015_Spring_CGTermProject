...
void getRatio2(Vec p, Vec a, Vec b, Vec c, float &u, float &v, float &w)
{
...
Vec DirectionalLight::applyLight(Face f, Vec pos, Object obj, Face hitNormal)
{
	Vec v[3];
	v[0] = f.v1;
	v[1] = f.v2;
	...
	if (!obj.isSphere)
	{
		...
		float beta;
		float gamma;
		getRatio2(pos, v[0], v[1], v[2], alpha, beta, gamma);
